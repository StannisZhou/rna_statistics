...
class RNA(object):
    ...
    def evaluate_local_ambiguities(self):
        self.local_ambiguities = get_local_ambiguities(
            self.primary_structure, self.r, self.case
        )
    ...
    def label_locations_using_secondary_structures(self):
        ...
        for key in ['comparative', 'mfe']:
            ...
            for ii in range(N - r + 1):
                n_paired_nucleotides[key][ii] = np.sum(
                    paired_indicator[key][ii : ii + r]
                )

            masks['single'][key] = n_paired_nucleotides[key] == 0
            masks['double'][key] = n_paired_nucleotides[key] == r
            masks['transitional'][key] = (n_paired_nucleotides[key] < r) * (
                n_paired_nucleotides[key] > 0
            )
            total_locations = 0
            for region in masks:
    ...
    def get_mean_ambiguity(
        ...
        mean_ambiguities[non_empty_mask_ind] = np.average(
            local_ambiguities_list[non_empty_mask_ind],
            axis=1,
            weights=mask[non_empty_mask_ind],
        )
    ...
    def get_ambiguity_index(self, key, index):
        ...
        if index == 'T-S':
            ...
            ) - self.get_mean_ambiguity(key, 'single', local_ambiguities_list)
        else:
            ambiguity_index = self.get_mean_ambiguity(
                key, 'double', local_ambiguities_list, True
            ) - self.get_mean_ambiguity(key, 'single', local_ambiguities_list, True)
    ...
    def get_alpha_index(self, key, index, shuffles=None):
        ...
        if index == 'T-S':
            ...
            ) - self.get_mean_ambiguity(key, 'single', shuffle_ambiguities)
        else:
            shuffle_ambiguity_index = self.get_mean_ambiguity(
                key, 'double', shuffle_ambiguities, True
            ) - self.get_mean_ambiguity(key, 'single', shuffle_ambiguities, True)
...
def get_local_ambiguities(primary_structure, r, case):
    ...
    segments_list = [''.join(primary_structure[ii : ii + r]) for ii in range(N - r + 1)]
